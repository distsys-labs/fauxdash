...
function applyWhen<T> (fn: (...args: any[]) => T | Promise<T>, args?: unknown[]): T | Promise<T> {
...
function bindAll<T extends object> (obj: T): void {
  const target = ((obj as any).prototype ? (obj as any).prototype : obj) as AnyRecord;
  const cast = obj as AnyRecord;
...
function clone<T> (source: T, target?: T): T {
...
function contains<T> (list: T[] | null | undefined, value: T): boolean {
  return Boolean(list && list.indexOf(value) >= 0);
}

function defaults<T extends AnyRecord> (target: T, ...sources: Array<Partial<T>>): T {
...
function each<T extends AnyRecord> (obj: T, iterator: (value: any, key: string, index: number) => void): void {
  if (!obj) {
    return;
  }
  getKeys(obj)
...
function exists (x: unknown): boolean {
  return x !== undefined && x !== null;
}

function filter<T> (list: T[], predicate: (value: T) => boolean = (y) => Boolean(y)): T[] {
  return list.reduce((acc, x) => predicate(x) ? acc.concat(x) : acc, [] as T[]);
}

function find<T> (list: T[], predicate: (value: T) => boolean = (y) => Boolean(y)): T | undefined {
  if (list.length === 0) {
    return undefined;
  }
  let found = false;
...
function flatten<T> (list: Array<T | T[]>): T[] {
  return list.reduce((acc: T[], b) =>
    acc.concat(Array.isArray(b) ? flatten(b as T[]) : b as T)
  , []);
}

function future<T = void> (): { promise: Promise<T>; resolve: (value: T | PromiseLike<T>) => void; reject: (reason?: any) => void } {
...
function getArguments (fn: Function): string[] {
...
function getPrototypeKeys (obj: AnyRecord): string[] {
...
function has (obj: AnyRecord | null | undefined, key: string): boolean {
...
function isEqual (a: any, b: any): boolean {
  if (isObject(a) && isObject(b)) {
    return reduce(a, (acc: boolean, v: any, k: string) => acc && isEqual(v, b[k]), true);
...
function isPromisey (x: any): boolean {
...
function last<T> (list: T[] | null | undefined): T | undefined {
  return list && list.length >= 0 ? list[list.length - 1] : undefined;
}

function lift<T> (asyncFn: (...args: any[]) => void): (...params: any[]) => Promise<T> {
  const lifted = function (...params: any[]) {
...
function map<T extends AnyRecord, R> (obj: T, fn: (value: T[keyof T], key: string, index: number) => R): R[] {
  if (!obj) {
    return [];
  }
...
function mapCall (method: (...args: any[]) => any, map: Record<string, string> | true | false): (actor: any, message: any) => any {
  const argumentList = getArguments(method).slice(1);
  if (map === false) {
    return method as any;
  } else {
    return function (actor: any, message: any) {
      const appliedArgs = [actor];
      const messageKeys = getKeys(message);
      argumentList.forEach((arg) => {
        const prop = (map && (map as Record<string, string>)[arg]) ? (map as Record<string, string>)[arg] : arg;
...
function matches<T> (filterValue: T): (value: T) => boolean {
  return isEqual.bind(null, filterValue);
}

function melter<T extends AnyRecord> (target: T | null | undefined, ...args: AnyRecord[]): T {
  const merged = ((target === undefined || target === null) ? ({} as T) : target);
...
function memoize<T extends (...args: any[]) => any>(fn: T): (...args: Parameters<T>) => ReturnType<T> {
  const set = new WeakMap<object, ReturnType<T>>();
  const memod = function (...args: Parameters<T>) {
    const key = args as unknown as object;
    if (set.has(key)) {
      return set.get(key) as ReturnType<T>;
    } else {
...
function merge (...objects: AnyRecord[]): AnyRecord | null | undefined {
  let copy: AnyRecord | null | undefined = {};
  let head: AnyRecord | null | undefined;
...
function mergeTwo (source: AnyRecord | null | undefined, target: AnyRecord | null | undefined): AnyRecord | null | undefined {
  const tag = getObjectTag(source);
  if (source == null || typeof source !== 'object') {
    return source;
  } else if (!isObject(source) && !Array.isArray(source)) {
...
function omit<T extends AnyRecord> (obj: T, ...keys: Array<string | string[]>): AnyRecord {
  const list = flatten(keys as Array<string | string[]>);
  return reduce(obj, (o: AnyRecord, v: any, k: string) => {
...
function parseFunction (fn: Function): { name: string | undefined; arguments: string[]; body: string | undefined } {
  const source = fn.toString().replace(NYC_DEFAULT_REGEX, '');
...
function reduce<T extends AnyRecord, R> (obj: T, fn: (acc: R, value: T[keyof T], key: string, index: number) => R, acc: R): R {
  if (!obj) {
    return acc;
  }
...
function sequence (...args: Array<(() => any) | Array<() => any>>): Promise<any[]> {
  const calls = flatten(args as Array<() => any>);
...
function sortBy<T extends AnyRecord> (list: T[], prop: string): T[] {
  list.sort((a, b) => {
...
function transform<T extends AnyRecord> (obj: T, aliases: Record<string, string>, ...omitKeys: Array<string | string[]>): AnyRecord {
  const list = flatten(omitKeys as Array<string | string[]>);
  return reduce(obj, (o: AnyRecord, v: any, k: string) => {
    if (o === undefined || o === null) {
      return o;
    }
...
function trim (list: Array<string | null | undefined>): string[] {
...
function uniq<T> (original: T[]): T[] {
  return original.reduce((acc: T[], x) => {
    if (!contains(acc, x)) {
      acc.push(x);
    }
    return acc;
...
function unique<T, I> (list: T[], identity: (item: T) => I = (x) => x as unknown as I): T[] {
  const ids: I[] = [];
  return list.reduce((acc: T[], item) => {
...
function values<T extends AnyRecord> (obj: T): Array<T[keyof T]> {
  if (!obj) {
    return [];
  }
  return Object.keys(obj as Object).map((k) => obj[k]);
}

function without<T> (a: T[], b: T[]): T[] {
  const a1 = uniq(filter(a));
  const b1 = uniq(filter(b));
  return a1.reduce((acc: T[], x) => {
    if (!contains(b1, x)) {
